...
split = split(Hitters$Salary, 0.75)
...
for (i in 1:length.lambdas) {
        boost.hitters = gbm(Salary~., data=salary.train, distribution="gaussian", n.trees=1000, shrinkage=lambdas[i])
        train.pred = predict(boost.hitters, salary.train, n.trees=1000)
        test.pred = predict(boost.hitters, salary.test, n.trees=1000)
        train.errors[i] = mean((salary.train - train.pred)^2)
        test.errors[i] = mean((salary.test - test.pred)^2)
}
