...
categoryFreq = function(old.frequencies.df) {
  ...
  numParks = c(1:56)        # do for each park
  categoryColumns = c(5:18) # do for each category (mammal, bird, etc.)
  categories = unique(species$Category) # vector of category strings
  for(parkIndex in numParks) {                    # do for each park
    parkName = old.frequencies.df[parkIndex, 1]   # get park name
    temp.data = subset(species, species$Park.Name == parkName)
    for(currentColumn in categoryColumns) {
      for(currentCategory in categories) {     # do for each category (bird etc.)
        category = categories[currentCategory] # get category
        old.frequencies.df[parkIndex, currentColumn] = sum(temp.data$Category == category)
      }
    }
      # ^ sum all observations belonging to current category in current park
    }
