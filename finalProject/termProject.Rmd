...
kmeans.out = kmeans(latitude.fungi, 3, nstart = 50)
cluster::clusplot(latitude.fungi, kmeans.out$cluster, shade = TRUE, color = TRUE, main = "Latitude and Fungi", xlab = "Latitude", ylab = "Fungi")
```

For reference, park latitude
...
```{r}
hclust.complete = hclust(dist(latitude.fungi), method = "complete")
hclust.avg = hclust(dist(latitude.fungi), method = "average")
hclust.single = hclust(dist(latitude.fungi), method = "single")
...
cluster::clusplot(plants, kmeans.out$cluster, labels = 2, main = "Vascular and Nonvascular Plants", col.p = kmeans.out$cluster, xlab = "Vascular Plants", ylab = "Nonvascular Plants")
```
