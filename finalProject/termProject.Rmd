...
Let's remove dimensions we likely won't get much use out of and create a dataframe with everything we'll need, with features to be added.
...
```{r}
categoryFreq = function(parkName, category) {
  temp.data = subset(species, species$Park.Name == parkName)
  frequency.df[parkIndex, 5] == sum(temp.data$Category == category)
