...
TV: There is no relationship between TV advertising and sales.
    The p-value is below 0.01, so we may reject the null hypothesis.
...
newspaper: There is not relationship between newspaper advertising and sales.
           The p-value is about 0.86, which is quite high. Thus, we may retain
           the null hypothesis. There is no relationship between newspaper ads 
           and sales.
...
male: (gender = 0)   50 + 20 k<sub>1</sub> + 0.07 k<sub>2</sub> + 0.01(k<sub>1</sub> * k<sub>2</sub>) 

female: (gender = 1) 50 + 20 k<sub>1</sub> + 0.07 k<sub>2</sub> + 35 + 0.01(k<sub>1</sub> * k<sub>2</sub>) - 10k<sub>1</sub>
...
Sales = β<sub>0</sub> + β<sub>1</sub>Price + β<sub>2</sub>UrbanYes + β<sub>3</sub>USYes
...
β<sub>0</sub> = -1 and β<sub>1</sub> = 0.5
...
β^<sub>0</sub> is closer to 0 than β<sub>0</sub>

β^<sub>1</sub> > β<sub>1</sub>
...
Based on the provided statistics, the quadratic term has no effect on the model fit. RSE, R<sup>2</sup>, and F-statistic are all identical in the original and the quadratic model.
...
Here, β^<sub>0</sub> = -1.00456 and β^<sub>1</sub> = 0.49701

Both values have decreased, with β^<sub>1</sub> getting closer to 0.
...
β^<sub>1</sub> is significantly closer to 0, while β^<sub>0</sub> has descender further in the negative direction.
